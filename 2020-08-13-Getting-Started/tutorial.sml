...
val aUnit       = () (* empty product type *); (* unit *)
...
val origin : real * real = (0.0, 0.0);
 ...
 (*
    val origin : real * real = (0.0, "0.0"); throw error since real*real was expected but got real*string
 *)
...
fun fact n = if n <= 0 then 1 else n * fact (n - 1); (* Notice the recursive use of fact *)
...
(*call functions as x = curry add; x 1 2;*)
...
(*
call functions as uncurry addp
*)
...
(*
Excercise: reversed fold
*)
fun foldr f x y = 
        if null y 
        then x 
        else 
            foldr f (f (hd(rev(y))) x) (rev(tl(rev(y))))
